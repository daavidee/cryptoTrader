      ...
      try {
        ...
        if (data.status != undefined) {
		  if (data.status == 0) {
			...
			callback(_this.createError(data.status, data.statustxt), null)
		  }
        }
        else {
			callback(false, data)
...
Vircurex.prototype.apiCallWithToken = function(command, key, params, callback) {
  ...
  if (command == 'create_released_order') var tokenItems = [key, this.username, time, id, 'create_order']
  else var tokenItems = [key, this.username, time, id, command]
  
  var urlVars = ''
  params.forEach(function(v) {
    if ((v[0] == 'otype') && (command != 'delete_order')) {
	  urlVars+= '&'+v[0]+'='+v[1]
	}
	else {
      tokenItems.push(v[1])
